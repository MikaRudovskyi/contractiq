...
import {
  ...
  FileStack, CreditCard, Settings, LogOut, BarChart3, X
} from 'lucide-react';
...
const NavItem: React.FC<{
  ...
  badgeWarn?: boolean;
  onClick?: () => void;
}> = ({ to, icon, label, badge, badgeWarn, onClick }) => {
  return (
    <NavLink
      to={to}
      onClick={onClick}
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* Затемнення позаду меню на мобільному — клік по ньому закриває меню */}
      ...
      <aside className={`sidebar ${isOpen ? 'sidebar-open' : ''}`}>
        <div className="sidebar-logo">
          ...
          <button className="sidebar-close-btn" onClick={close} aria-label="Закрити меню">
            <X size={18} />
          </button>
        </div>
        ...
        <nav className="sidebar-nav">
          ...
          <NavItem to="/dashboard" icon={<LayoutDashboard size={16} />} label="Дашборд" onClick={close} />
          <NavItem to="/contractors" icon={<Users size={16} />} label="Підрядники" onClick={close} />
          <NavItem to="/contracts" icon={<FileText size={16} />} label="Договори" onClick={close} />
          <NavItem
            to="/work-orders"
          ...
          <NavItem
            ...
            onClick={close}
          />
          <NavItem to="/analytics" icon={<BarChart3 size={16} />} label="Аналітика" onClick={close} />

          <span className="sidebar-section-label">Система</span>
          <NavItem to="/settings" icon={<Settings size={16} />} label="Налаштування" onClick={close} />
        </nav>
